...
# Блок «Источники» — сплошь чужие заглавия: «Cross-Origin Resource Sharing
# (CORS)», «Session ID Properties», «Introspection». Заглавие цитируется
# буквально (7.4), поэтому канон написаний на него не распространяется.
# Признак — заголовок, а не номер: номер у двух скелетов разный.
SOURCES_RE = re.compile(r"^##\s*\d+\.\s*Источники", re.M)
...
# Блок «Источники» говорит не о материале страницы, а о чужих документах, и
# материал страницы заканчивается его заголовком (решение оператора 2026-08-31:
# блока «Дальше» в исходниках больше нет, ссылку вперёд генерирует сборка).
# Аббревиатура в сноске — не первое употребление термина, и раскрывать её
# незачем. Признак — заголовок, а не номер: номер у двух скелетов разный.
MATERIAL_END_RE = SOURCES_RE
...
def first_hits(pages, regexes, field="prose", material_only=False):
    ...
    При `material_only` берётся материал страницы своими словами: всё до блока
    «Источники», без цитат и заглавий в «ёлочках». Причина та же, что у
    `canon_scope`, и записана
