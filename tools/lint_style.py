...
# Блоки 13 «Дальше» и 14 «Источники»: указатели на другие темы и чужие заглавия.
# Мерить их длиной предложения бессмысленно — заглавие длинное по своей природе.
...
def check_code_punct(path, doc) -> list[Finding]:
    ...
    for m in mdtext.CODE_SPAN_RE.finditer(text):
        ...
        if SCHEME_RE.fullmatch(inner) or not inner.strip(",.;: "):
            ...
            # («файловая система получает `.`»), а не пунктуация фразы.
            pass
...
def words(doc, a: int, b: int) -> int:
    ...
    return len(doc.prose_spans[a:b].split())
