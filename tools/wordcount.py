...
`текст`  — вердиктная: проза без служебного аппарата. Из подсчёта исключены
           frontmatter; заголовок h1 и блок идентификации под ним
           (`**AG-…**` … `маппинг:`) — он дословно повторяет данные шапки;
           абзац «Состав блоков — …»; абзацы «Описание схемы.»; блок 14
...
NORM = {"L1": (2500, 3500), "L2": (800, 1200), "L3": (300, 500)}
...
# Заголовок и блок идентификации: h1, за ним абзац `**AG-XXX-NN** · уровень …`
# до пустой строки. По `SCHEMA.md` § 2 это вторая копия шапки для читателя
# бумажной распечатки, а не текст темы.
...
def counts(text: str) -> tuple[int, int, int]:
    ...
    core = SERVICE.sub("", IDENT.sub("", prose))
    return (
