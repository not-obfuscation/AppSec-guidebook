...
`текст`  — вердиктная: проза без служебного аппарата. Из подсчёта исключены
           ...
           сначала: …» — это навигация; абзацы «Описание схемы.»; блок
...
`проза`  — справочная: тело без листингов, без ответов под `<details>` и без
           блока «Источники». Метрика первой приёмки — по ней считаны находки
           Ф-09 и Ф-42, поэтому она остаётся в выводе.
...
# (свод 4.2) он двенадцатый, а не тринадцатый, и номер здесь был бы
# сведением об одном скелете из двух.
SOURCES = re.compile(r"^## \d+\.\s+Источники\s*$.*\Z", re.M | re.S)
