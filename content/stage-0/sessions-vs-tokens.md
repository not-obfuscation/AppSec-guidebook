...
1. По структуре значения: несколько частей, разделённых точкой, каждая
   декодируется из base64url в осмысленный JSON (RFC 7519 § 3). Ссылочный
   токен декодированию не поддаётся, потому что смысла в нём нет.
