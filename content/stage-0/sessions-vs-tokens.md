...
asvs: ['v5.0-7.2.2', 'v5.0-7.2.3', 'v5.0-7.2.4', 'v5.0-7.4.1',
       'v5.0-7.4.2', 'v5.0-9.1.2', 'v5.0-9.1.3']
wstg: []
owasp: []
labs: []
...
Что прочитать сначала: `http-basics`, `cookies`.

Состояние входа между запросами держится двумя способами: записью на сервере,
на которую ссылается идентификатор, или самодостаточным токеном. Выбор виден
в момент, когда доступ надо отнять до срока. Пример. HR-система выдаёт
самодостаточные токены со сроком восемь часов; учётную запись отключили
в админке. Вот запрос из её сессии после отключения:

**Запрос**

```http
GET /reports/payroll HTTP/1.1
Host: hr.example
Authorization: Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJlbXAtNzcxIn0...
```
...
Ни подделки, ни украденного пароля здесь нет: токен настоящий, подпись верна,
срок не истёк. Приложение и не обращалось никуда, чтобы это выяснить, —
самодостаточный токен для того и выбран, чтобы проверка ничего не спрашивала у
хранилища. Отключённая учётная запись живёт в базе, а токен о ней не знает: он
несёт утверждения, сделанные в момент выпуска. Будь сессия
ссылочной, запись погасили бы вместе с учётной записью, и следующий запрос
получил бы отказ.

Здесь видно главное свойство модели: токен правдив, но не в курсе того, что
изменилось после его выпуска. Ответьте себе, не читая дальше: изменился бы
ответ сервера, будь токен выдан не за час до отключения, а за минуту?

Каждый HTTP-запрос приходит к серверу как первый: о предыдущем протокол не
помнит ничего. Чтобы приложение узнавало вошедшего, состояние переносит само
сообщение — cookie или поле заголовка. Способов два, и различает их место, где
лежит смысл сессии: запись на сервере или сама строка, которую держит клиент.

Пока пользователь ходит по приложению, обе модели снаружи выглядят одинаково.
Разница видна в одном месте: когда доступ надо отнять до срока.

К этому запросу тема возвращается трижды. «Как выглядит в коде» показывает, что
смотрят в разборе такого токена. «Как чинится» — почему правка ссылочной
модели ему не помогает. «Ловушка» объясняет, почему удаление токена у клиента
ничего не отменяет.

Атаки на JWT разбираются в теме 1.7.02, session fixation и некорректная
инвалидация — в теме 1.6.04; здесь только модель.

## 0. Коротко

То же в терминах, которыми это обсуждают. Состояние между запросами переносится
самим сообщением. Два способа различаются тем, где лежит смысл. **Ссылочный
...
прав. Оба вида — единственные, которые ASVS v5.0-7.2.2 допускает для
управления сессией: токен порождается динамически, а статический секрет вроде
ключа API в этой роли не допускается.

**Зачем это в работе AppSec-инженера.** Вход в приложение вы делали: cookie с
идентификатором или токен в поле заголовка, библиотека, пара строк в
обработчике. Что происходит с этой сессией при выходе, проверять было незачем —
кнопка «выйти» работала. Вопрос «как у вас устроены сессии» звучит на каждом
дизайн-ревью нового сервиса, и ответ «JWT» на него не отвечает. Отвечает то,
что вы спросите следом: где проверяется подпись, что происходит при выходе,
какие алгоритмы приняты.
...
**Откуда это взялось.** HTTP не связывает два запроса: каждая пара «запрос —
ответ» независима от остальных. Чтобы приложение помнило вошедшего, состояние
стали держать на сервере, а клиенту отдавать короткий идентификатор. Чем этот
идентификатор стал, OWASP предупреждает отдельно: на время сессии он
равносилен самому сильному способу аутентификации в приложении. Состояние на
сервере означает хранилище, в которое смотрит каждый процесс, обслуживающий
запрос, — и второй способ переносит состояние в само сообщение. JWT для этого и задуман: компактная запись
утверждений для мест, где мало байтов, вроде заголовка `Authorization`. Платой
стал отзыв: сервер, который ничего не хранит, ничего и не забывает.
...
такому значению три требования:

- имя не раскрывает стек: `PHPSESSID`, `JSESSIONID` и `ASP.NET_SessionId`
  опознаются по имени;
- значение порождается CSPRNG;
- смысла в самом значении нет — иначе он раскрывается декодированием.
...
записью на бэкенде. Самодостаточному нужно «решение вроде списка завершённых
токенов, отказа от токенов, выпущенных раньше определённой для пользователя
даты, или ротации ключа подписи для этого пользователя».
...
Первые два относятся к ссылочной модели, третье и четвёртое — к разбору такого
токена, как в примере начала темы.
...
отметить событие как подозрительное. Найдите в четырёх строках ниже место, где
приложение поверило значению, которого не выдавало, — до того, как читать
выноски.
...
1. Идентификатор принимается ещё и из параметра запроса — значит, попадает в
   адрес страницы (URL, Uniform Resource Locator). А оттуда — в журналы, в
   историю браузера и в поле `Referer`.
...
отзыв. Токен из начала темы после этой правки работал бы те же семь часов.
...
Рассуждение звучит убедительно: раз токен подписан и срок в нём указан,
отдельный выход из системы не нужен — достаточно удалить токен у клиента.
Пользователь нажал «выйти», приложение стёрло токен из хранилища браузера.
Доступ закрыт?

**Нет.** Удаление на клиенте не отменяет проверяемость токена.
...
Случай, который это ломает, стоит в начале темы. Учётную запись отключили, а
токен со сроком в восемь часов выдали за час до этого: семь часов доступ
действителен. ASVS v5.0-7.4.2 требует завершать все активные сессии как раз
...
2. Verify that приложение не заводит сессию под идентификатором, который оно
   не выдавало.
3. Verify that сессией управляет динамически порождённый токен — ссылочный
   или самодостаточный, — а не статический секрет или ключ API
   (ASVS v5.0-7.2.2).
4. Verify that при аутентификации и при любой смене уровня привилегий
   выдаётся новый токен, а прежний завершается (ASVS v5.0-7.2.4).
5. Verify that ссылочные токены порождаются CSPRNG и несут не менее 128 бит
   энтропии (ASVS v5.0-7.2.3).
6. Verify that проверка самодостаточных токенов принимает алгоритмы по
   явному списку, в котором нет `none` (ASVS v5.0-9.1.2).
7. Verify that у самодостаточных токенов заведён работающий механизм отзыва —
...
**Критерий выполнения.** Паспорт содержит:

- имя и способ переноса токена;
- ссылочный он или самодостаточный, с обоснованием;
- принимает ли приложение идентификатор из URL;
- меняется ли значение после входа;
- что происходит с ним после выхода;
- чем ограничен срок жизни.

Против каждого пункта записано наблюдение.
...
**Маркеры уверенности.** Модель JWT, Unsecured JWT и назначение формата —
компактная запись утверждений для мест, где мало байтов (§ 1), — по
спецификации (RFC 7519, открыт лично 2026-08-22). Свойства идентификатора
сессии, два механизма его приёма и равносильность самому сильному способу
аутентификации — по документации (Cheat Sheet, открыт лично 2026-08-22). Требования 7.2.2, 7.2.3, 7.2.4, 7.4.1, 7.4.2, 9.1.2, 9.1.3 сверены с
