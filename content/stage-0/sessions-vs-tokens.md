...
**Что это такое.** Каждый HTTP-запрос приходит к серверу как первый: о
предыдущем протокол не помнит ничего. Чтобы приложение узнавало вошедшего,
состояние переносит само сообщение — cookie или поле заголовка. Способов два, и
различает их место, где лежит смысл сессии: запись на сервере или сама строка,
которую держит клиент.

Отсюда растёт вся тема. Пока пользователь ходит по приложению, обе модели
снаружи выглядят одинаково. Разница видна в одном месте: когда доступ надо
отнять до срока.

**Пример: сотрудника отключили, а он ещё работал.** Учётную запись уволенного
выключили в админке в 10:05 — кнопка сработала, статус сменился на
«отключена». В 10:40 из его сессии открыли отчёт по зарплатам. Сессия держалась
самодостаточным токеном со сроком восемь часов, выданным в 09:20.
...
несёт утверждения, сделанные в момент выпуска, то есть в 09:20. Будь сессия
