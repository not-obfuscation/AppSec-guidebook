...
безопасности, а файлы по схеме `file:///` браузеры обычно считают opaque
origin.
...
чужого документа. **Путь** — окно, кадр или сетевой запрос. **Сток** — то, что
...
дефект в трёх строках, и через них прошло сообщение из начала темы. Прочитайте
их до выносок и назовите сами, каких двух проверок здесь нет.
...
Три строки, а правило из них общее: применять присланное к состоянию страницы
можно только после ответа на вопрос, кто это прислал. Прежде чем читать
дальше, решите: был бы дефектом обработчик, который то же сообщение разбирает,
но ничего в странице не меняет?
...
состояния. Ни один из трёх шагов не заменяет другие. Читайте листинг с тремя
планами: найти, где отсекается чужой адрес; найти, где отсекается тело не той
формы; понять, как разбор отделён от применения. Последнее гарантирует, что
неполное значение ничего не меняет. Сообщение из начала темы не проходит
первый же шаг: адреса чужого сайта в списке нет.
...
проверку так, что компилятор начинает считать значение проверенным. Читайте с
двумя планами: найти, где здесь сверка origin, и сравнить предикат с
приведением типа из блока 5.
...
той, с которой тема началась. Поставьте себя на место такого ревьюера и
ответьте до чтения дальше: дефект устранён и функциональность на месте?
