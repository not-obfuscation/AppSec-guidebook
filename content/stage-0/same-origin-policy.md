...
tags: [browser, origin]
cwe: [CWE-346]
asvs: ['v5.0-3.5.1', 'v5.0-3.5.4', 'v5.0-3.5.5']
wstg: []
owasp: []
...
**AG-PROTO-05** · уровень **L1** · время 90 мин (теория 40 / лаба 35 /
самопроверка 15, оценка) · reviewed: 2026-08-23 · проверено на: HTML Living
Standard (правка 21 августа 2026), MDN (правка 29 ноября 2025), ASVS v5.0.0,
chrome-headless-shell 152.0.7977.42, Semgrep 1.174.0, CWE 4.20
пререквизиты: `http-basics`, `url-and-encoding` · маппинг: CWE-346 ·
ASVS v5.0-3.5.1 · ASVS v5.0-3.5.4 · ASVS v5.0-3.5.5

Состав блоков — полный по уровню L1 (`PLAYBOOK.md` 3.1): удалений нет, порядок
канонический. Уровень поднят с L2 решением оператора от 2026-08-23: на теме
стоят `cors` и `csp`. Обходы политики разбираются в подразделе 1.4, подделка
межсайтовых запросов — в подразделе 1.5.
...
CORS (Cross-Origin Resource Sharing), CSP (Content Security Policy) и
cookie-атрибуты — стоят поверх этой модели и без неё не читаются.
...
свободно. MDN приводит ровно этот случай: сайт из интернета читает чужую
веб-почту, в которую вы вошли, или страницу корпоративной сети, куда
...
что наружу стенд не обращается. Приёмы ниже привязаны к этому стенду и
...
состояния. Ни один из трёх шагов не заменяет другие.
...
> задуманному получателю нет, а при несовпадении явного значения с origin
...
адрес: тот, откуда приходят сообщения виджета.
...
- `csp` — как страница ограничивает саму себя в том, что политика ей
  разрешает.
- `security-headers` — чем закрывается встраивание в кадр, которое политика
  пропускает.
...
2. HTML Standard, § 9.3 Cross-document messaging, WHATWG, Living Standard
   ...
   § 9.3.1 Introduction, § 9.3.2.1 Security risks, § 9.3.3 Posting messages.
...
**Скоропортящийся слой.** Все три источника живые. HTML Standard правится
...
модель cookie и пример с чужой веб-почтой и корпоративной сетью — по
документации (MDN, открыта лично 2026-08-22). Требования
3.5.1, 3.5.4 и 3.5.5 сверены с текстом ASVS v5.0.0 (открыт лично 2026-08-22).
...
— из этих прогонов. Внешнего технического ревью у темы нет: миссии оно
недоступно.
