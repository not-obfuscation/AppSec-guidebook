...
**Что это такое.** Браузер держит страницы разных сайтов врозь: скрипт одного
сайта не читает данные другого. Правило разделения называют политикой одного
источника, same-origin policy. Источник — тройка из адреса страницы: схема,
хост и порт. Две страницы с одинаковой тройкой браузер считает своими друг для
друга, с разными — чужими.

Отсюда растёт вся тема. Запрет касается чтения: чужую страницу и чужой ответ
сервера скрипт не прочитает. А отправить запрос, встроить чужую страницу кадром
и послать в её окно сообщение он может — политика этого не закрывает.

**Пример: чужая страница поменяла настройки в кабинете.** Кабинет пользователя
встраивает виджет курсов, тот живёт на отдельном имени. Настройки виджет
присылает в главное окно сообщением, а кабинет их применяет. То же самое делает
страница на чужом сайте: она встраивает кадром сам кабинет и посылает ему
сообщение той же формы.
...
- `postmessage-vulns` — чем ломаются проверки origin; `clickjacking` — что
  политика разрешает: показывать приложение в кадре.
...
— из этих прогонов. Фрагмент чужой страницы во введении — переложение опыта 5
в вид обычной страницы, и в этом виде он не запускался. Внешнего технического ревью у темы нет: автору оно
недоступно.
