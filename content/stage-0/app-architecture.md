...
Между браузером и приложением стоят прокси и кеши, и каждый из этих узлов
правит запрос по дороге. Решение о доступе по адресу клиента на ревью
проверяют вопросом: откуда приложение этот адрес узнало. Пример. Панель
администратора закрыта «только для внутренней сети»: приложение смотрит,
лежит ли адрес клиента в сети `10.0.0.0/8`. Вот запрос, который такая
проверка пропускает:
...
Ни пароль, ни сессию здесь не подбирали: запрос пришёл от обычного
пользователя с настоящей сессией и настоящими правами. Строку
`X-Forwarded-For` он набрал сам — это обычное поле заголовка, и поставить его
вправе кто угодно. Приложение приняло поле за сведение от своего прокси, потому
что отличить одно от другого ему нечем.

Приложение на ревью — не один узел, а цепочка. Часть кода работает в браузере,
часть на сервере; перед сервером стоят прокси и кеши, за ним — один или
несколько API. Каждый узел цепочки правит запрос по дороге: что-то дописывает,
что-то стирает, что-то отдаёт из своей памяти, не спрашивая приложение.

Клиента напрямую приложение не видит. Всё, что оно о клиенте знает, ему
рассказал сосед по цепочке — текстом, в поле заголовка. А текст в это поле
пишет кто угодно.

К этому запросу тема возвращается трижды. «Как выглядит в коде» показывает
такую проверку изнутри, «Как чинится» — что меняют в цепочке и в приложении.
«Ловушка» объясняет, почему адрес за прокси не считается адресом клиента.

Тема несущая: она задаёт карту, на которую ложатся разборы этапов 1–5.

## 0. Коротко

То же в терминах, которыми это обсуждают. Логика разложена между браузером и
сервером — по-разному в зависимости от способа рендеринга. Перед приложением
стоит цепочка из обратного прокси, сети доставки и кешей. За ним — один или
несколько API. Для разбора важны два следствия. Всё, что уехало на клиент,
доступно атакующему целиком. А всё, что приложение знает о клиенте, оно узнаёт
от промежуточных узлов, а не напрямую.

**Зачем это в работе AppSec-инженера.** Приложение за обратным прокси вы
поднимали и адрес клиента в журнале видели. Откуда этот адрес взялся — из
соединения или из текста, дописанного прокси, — при настройке спросить незачем:
работает и так. На ревью это первый вопрос. Рядом второй: где именно
выполняется код, который вы читаете. Без карты узлов находка либо не
воспроизводится, либо адресуется не той команде.
...
главное следствие темы. Сервер узнаёт клиента из текста, который поставил
промежуточный узел, а не из самого соединения.
...
Главный факт темы принадлежит RFC 7239. Поле `Forwarded` **нельзя считать
верным**: его может изменить, по ошибке или намеренно, каждый узел на пути к
серверу, включая сам клиент. У подхода «проверить прокси и внести их в
...
узлов цепочки. Прочтите фрагмент как ревьюер, до выносок: отметьте строки,
которые переписали бы, и причину у каждой.
...
2. Решение о доступе принимается по этому значению. Внутренняя сеть
   «опознаётся» строкой, присланной снаружи, — так и открылась админка в
...
Соберите три выноски вместе — они держатся на одном правиле. Текст поля
пересылки становится сведением только после того, как его переписал ваш узел,
а до того он не достовернее любой строки из тела запроса. Проверочный вопрос:
если бы приложение читало стандартизованное `Forwarded` вместо
`X-Forwarded-For`, какая из трёх находок исчезла бы?

**Что искать глазами.** Признак — то, что делают с полем, а не его имя. Запись
...
Дефект из примера чинится в двух местах сразу: на внешнем узле цепочки и в
самом приложении.
...
3. Адрес перестал быть единственным основанием: сетевой признак дополнен
   проверкой роли. Требование к сети — сужение доступа, а не контроль над ним.
...
клиента. Сначала прикиньте ответ сами: клиент сам написал в поле
`X-Forwarded-For` адрес офисной сети. Что увидит в этом поле приложение?
...
Контрпример уже прочитан. Адрес `10.0.0.7`, по которому открылась админка,
офисной сети не принадлежал: его написал в поле тот же, кто послал запрос.
Вторая ситуация, в которой это видно. Ограничение частоты считает попытки входа
по `X-Forwarded-For`. Атакующий меняет значение в каждом запросе — счётчик
никогда не достигает порога; в журнале остаются тысячи разных «клиентов».
Дефект при этом не в ограничителе: источник адреса не назван.
...
6. Возврат к теме `http-basics`: из каких частей собран HTTP-транскрипт
   и в какой из них прокси оставляет свои поля?
...
6. Из start-line, полей и тела, отделённых пустой строкой; поля пересылки
   вроде `X-Forwarded-For` живут в блоке полей запроса.
...
не говорит ничего; сеть доставки не описана ни одним из первых двух
