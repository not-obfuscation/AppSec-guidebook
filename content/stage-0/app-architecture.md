...
что отличить одно от другого ему нечем. Ничего взламывать не потребовалось.
...
пишет кто угодно. Стоит ответить себе сразу: что поменялось бы для приложения,
перезапиши внешний узел цепочки это поле своим значением?
...
работает и так. На ревью это первый вопрос, вместе со вторым: где именно
...
растёт главное следствие темы: сервер узнаёт клиента из текста, который
поставил промежуточный узел, а не из самого соединения.
...
узлов цепочки. Таких мест в фрагменте два — попробуйте назвать оба до разбора.
...
клиента. Проверьте его на запросе из начала темы, прежде чем читать разбор.
