...
**Что это такое.** Приложение на ревью — не один узел, а цепочка. Часть кода
работает в браузере, часть на сервере; перед сервером стоят прокси и кеши, за
ним — один или несколько API. Каждый узел цепочки правит запрос по дороге:
что-то дописывает, что-то стирает, что-то отдаёт из своей памяти, не спрашивая
приложение.

Отсюда растёт вся тема. Приложение не видит клиента напрямую. Всё, что оно о
клиенте знает, ему рассказал сосед по цепочке — текстом, в поле заголовка. А
текст в это поле пишет кто угодно.

**Пример: админка открылась из интернета.** Панель администратора закрыта
«только для внутренней сети»: приложение смотрит, лежит ли адрес клиента в сети
`10.0.0.0/8`. Проверка работает — из офиса панель открывается, из интернета нет.
Ночью в журнале появляется вход с адреса `10.0.0.7`, которого в офисной сети не
существует.
...
К этому запросу тема возвращается трижды. «Как выглядит в коде» показывает такую
проверку изнутри, «Как чинится» — что меняют в цепочке и в приложении. «Ловушка»
объясняет, почему адрес за прокси не считается адресом клиента.
...
Дефект из примера чинится в двух местах сразу: на внешнем узле цепочки и в самом
приложении.
