...
id: app-architecture
plan_id: t-0-0-10
title: 'Модель современного приложения: SPA + REST/GraphQL API, серверный рендеринг, reverse proxy, CDN'
summary: >
...
owasp: []
labs: []
sources: [webdev-rendering-on-the-web, mdn-proxy-tunneling, rfc7239-forwarded, rfc9111-http-caching]
reviewed: 2026-08-22
review_interval: 24
---
...
**AG-PROTO-10** · уровень **L2** · время 30 мин (теория 15 / задача 10 /
самопроверка 5, оценка) · reviewed: 2026-08-22 · проверено на: web.dev
«Rendering on the Web» (правка 5 января 2026), MDN (правка 9 ноября 2025),
RFC 7239 (июнь 2014), RFC 9111 (STD 98, июнь 2022)
пререквизиты: `http-basics`, `tls-and-proxy` · маппинг: CWE-348 · CWE-524

Состав блоков — по уровню L2 (`PLAYBOOK.md` 3.1). Блоки 2, 4, 7 и 8 удалены,
блок 11 сведён к задаче без лабы, порядок остальных сохранён. Тема несущая:
она задаёт карту, на которую ложатся разборы этапов 1–5.
...
уходит HTML, а не JavaScript; **клиентский рендеринг** — отрисовка в браузере
изменением DOM; **предварительный рендеринг** — прогон клиентского приложения
на этапе сборки, чтобы получить начальное состояние статическим HTML;
...
`CONNECT`; так клиент за прокси добирается до сайтов по TLS, и MDN сразу
...
список доверенных» спецификация называет две слабости: цепочка адресов,
перечисленных до того, как запрос пришёл на прокси, доверия не заслуживает; и
если связь между прокси и конечным узлом не защищена, данные изменит любой,
у кого есть доступ к сети.
...
для другого. Хранимый ответ переиспользуется, только если совпал целевой URI,
метод это позволяет, совпали поля запроса, названные в `Vary`, ответ не несёт
`no-cache` и при этом свеж, допущен к отдаче несвежим или успешно
провалидирован.
...
директивы, разрешающей такое хранение, — а таких директив ровно три:
...
адреса в журнал по `X-Forwarded-For` допустима и полезна; решение о доступе,
...
7. Verify that заголовок `X-Content-Type-Options: nosniff` выставлен на
   ответах, отдаваемых через сеть доставки (ASVS v5.0-3.4.4).
...
6. Браузер отвергает ответ с несколькими полями `Access-Control-Allow-Origin`
   — MDN называет эту причину отдельной строкой в списке отказов.
...
- Тема 1.1.07 — обход контроля доступа подменой полей пересылки.
...
(DoD 11.2 пункт 8). Причина зафиксирована аудитом: страница MDN перечисляет
поля пересылки, но об их подделке не говорит ничего, а сеть доставки не
описана ни одним из первых двух источников. Превышение отмечено в DoD как
невыполненный пункт.
...
Недостоверность поля `Forwarded` и две слабости списка доверенных — по
спецификации (RFC 7239, открыт лично 2026-08-22). Правила кеша, условия
переиспользования и требование к запросам с `Authorization` — по спецификации
(RFC 9111, открыт лично 2026-08-22). Цепочка узлов не разбиралась на живой
системе: стенда у миссии нет.
