...
Панель администратора закрыта «только для внутренней сети»: приложение
смотрит, лежит ли адрес клиента в сети `10.0.0.0/8`. Проверка работает — из
офиса панель открывается, из интернета нет. Ночью в журнале появляется вход с
адреса `10.0.0.7`, которого в офисной сети не существует.
