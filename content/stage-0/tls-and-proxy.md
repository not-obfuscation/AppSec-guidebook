...
id: tls-and-proxy
plan_id: t-0-0-09
title: 'TLS: рукопожатие в общих чертах, цепочка сертификатов, перехват трафика прокси'
summary: >
...
**AG-PROTO-09** · уровень **L2** · время 30 мин (теория 15 / задача 10 /
самопроверка 5, оценка) · reviewed: 2026-08-22 · проверено на: RFC 9846
(июль 2026), RFC 5280 (май 2008), документация Burp Suite (правка 20 августа
2026), ASVS v5.0.0
пререквизиты: `http-basics`, `security-headers` · маппинг: CWE-295 ·
CWE-319 · ASVS v5.0-12.3.2

Состав блоков — по уровню L2 (`PLAYBOOK.md` 3.1). Блоки 2, 4, 7 и 8 удалены,
блок 11 сведён к задаче без лабы, порядок остальных сохранён. Слабые и
устаревшие алгоритмы разбираются в теме 1.11.02, передача данных в открытом
виде — в теме 1.11.01; здесь модель и рабочий инструмент.
...
инструмент, и понимать его надо не по кнопкам, а по механизму: тот же
механизм в чужих руках означает чтение вашего трафика, а в коде приложения он
выглядит одной строкой отключённой проверки.
...
собственный сертификат удостоверяющего центра, и слушатели Proxy используют
его для согласования соединений TLS, выпуская сертификаты на каждый хост.
...
формулирует и обратную сторону: закрытый ключ не следует раскрывать никому,
потому что владеющий вашим сертификатом и ключом сможет перехватывать ваш
трафик HTTPS даже тогда, когда вы Burp не пользуетесь.
...
и четыре проверки звена — по спецификации (RFC 5280, открыт лично
2026-08-22). Устройство удостоверяющего центра Burp и предупреждение о
...
v5.0.0 (открыт лично 2026-08-22). Перехват не выполнялся: стенда у миссии нет.

**Число источников.** У темы пять собственных источников при норме два-три
(DoD 11.2 пункт 8). Причина зафиксирована аудитом реестра: TLS 1.3 выносит
проверку сертификата к RFC 5280, а обзорная страница прокси не содержит ни
процедуры перехвата, ни работы с удостоверяющим центром — обе вынесены на
подстраницы. Превышение отмечено в DoD как невыполненный пункт, а не скрыто.
