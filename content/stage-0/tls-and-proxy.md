...
Случай поучителен ровно этим: все проверки прошли честно, и трафик открыло
доверие, а не взлом. Ответьте, не читая механику ниже: изменилось бы
что-нибудь, выпусти прокси сертификат на своё собственное имя, а не на
`shop.example`?
...
безупречно и установлено при этом не с тем, с кем вы думали.
...
Ревьюер ищет три вещи, и все три — про проверку, а не про шифрование. В каждой
из трёх строк ниже проверка ослаблена по-своему: назовите, что именно перестаёт
проверяться, до того, как читать выноски.
...
**Чего не даёт.** Проверенная цепочка подтверждает связь имени с ключом. О
владельце этого имени она не говорит ничего. Не защищает она и от того, кто
законно попал в хранилище доверия, — включая перехватывающий прокси вашей же
организации. Стенд из начала темы после этой правки читает трафик по-прежнему:
якорь прокси из хранилища никуда не делся.

**Стоимость.** Внутренний удостоверяющий центр требует раздачи якоря на все
узлы и продления сертификатов по расписанию. Это регулярная эксплуатационная
работа. Просроченный внутренний сертификат кладёт связность так же надёжно, как
ошибка в коде.

## 9. Ловушка

Расхожее объяснение: если браузер не показал предупреждения, цепочка проверена
полностью. Проверьте его на стенде из начала темы: замок закрыт, предупреждений
нет — что именно было проверено?

**Объяснение описывает не то.** Проверено ровно то, что клиент решил проверять.
