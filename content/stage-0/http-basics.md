...
1. Verify that каждый маршрут объявляет разрешённые методы явно, а
   неиспользуемые методы заблокированы (ASVS v5.0-4.1.4).
...
5. Verify that `PUT` и `DELETE` недоступны на маршрутах, где приложение их не
   реализует.
6. Verify that `TRACE` отключён на всех узлах цепочки.
7. Verify that имена полей с подчёркиванием не отображаются в те же переменные
   приложения, что имена с дефисом (RFC 9110 § 17.10).
8. Verify that код ответа не раскрывает существование ресурса там, где это
