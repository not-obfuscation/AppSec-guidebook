...
В админке есть ссылка «удалить черновик» — обычная ссылка вида
`<a href="/page?do=delete&id=1024">`. Утром черновика нет, хотя на ссылку никто
не нажимал. В логе за ночь — один запрос.
...
Обмен по HTTP — это два текста: запрос от клиента и ответ от сервера. В первой
строке запроса стоит метод (`GET`, `POST`, `DELETE`) и адрес, в первой строке
ответа — трёхзначный код. Всё остальное — поля-заголовки и тело.
...
проверяет никто — ни браузер, ни фреймворк, ни прокси. Весь дальнейший разбор
стоит в этом зазоре.
