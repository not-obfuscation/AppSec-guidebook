...
Уровень **L2** · время 28 мин (теория 15 / задача 8 / самопроверка 5,
оценка)

HTTP — текстовый протокол обмена сообщениями: клиент шлёт запрос, сервер
отвечает. Запрос и ответ устроены одинаково: первая строка (у запроса — метод
и адрес, у ответа — трёхзначный код), поля-заголовки, пустая строка, тело.

Для проверяющего этот протокол — основной артефакт: отчёт сканера, тикет,
разбор инцидента — всё сводится к транскрипту запроса и ответа, и читать его
приходится ежедневно. Главное, что в нём видно, — расхождение между тем, что
запрос объявляет, и тем, что обработчик делает. Пример. В админке сайта лежит
недописанная статья, и удаляется она обычной ссылкой «удалить черновик». Вот
запрос, который уходит при её нажатии:

**Запрос**

```http
GET /page?do=delete&id=1024 HTTP/1.1
...
Метод `GET` по стандарту безопасен: он обязуется только читать. Здесь действие
выбрано параметром `do=delete`, и запись выполняется методом, объявленным
«только чтением». Обходчики и предзагрузка ходят по таким ссылкам сами, без
всякого злоумышленника. Метод — не команда и не проверка, а ярлык, который
пишет клиент. Совпадает ли он с действием обработчика, не проверяет никто:
ни браузер, ни фреймворк, ни прокси. Весь дальнейший разбор стоит в этом
зазоре.

Дальше: что обещает каждый метод и кому, как выглядит такой обработчик
в коде и как его чинить.
...
Запрос и ответ устроены одинаково:
start-line, поля-заголовки, пустая строка, тело. Метод в start-line объявляет
намерение клиента, трёхзначный код в ответе объявляет результат, поля несут всё
...
сверкой «метод — действие — код ответа». То же в терминах, которыми это
обсуждают.

**Зачем это в работе AppSec-инженера.** Обработчик HTTP-запроса вы писали.
Метод в нём, скорее всего, ни разу не сверяли с тем, что код делает на самом
деле: фреймворк разбирает запрос, автор пишет логику. Именно это расхождение и
есть предмет проверки, а видно оно в одном артефакте — в транскрипте запроса и
ответа. Пока вы не читаете транскрипт сами, вы читаете чужие выводы о нём:
отчёт сканера, текст тикета, скриншот от разработчика.
...
**Откуда это взялось.** Метод в start-line — объявление намерения клиента, и
поначалу это выглядело формальностью: сервер отвечает, что попросили, а
остальное его дело. Формальность перестала быть безобидной, когда по ссылкам
пошли автоматы: поисковые обходчики и предзагрузка дёргают каждый найденный
адрес, ничего не спрашивая. RFC 9110 называет причину различения прямо:
безопасные и небезопасные методы разделили затем, чтобы обходчики и кеши
работали без страха навредить. Обещание про намерение потому и обязывает
сервер: на нём построено поведение чужих программ, которые вашего кода не
видели.

Устройство сообщения видно на любой паре «запрос — ответ», поэтому возьмём
чтение заказа, где подвоха нет.
...
остальные спецификация помечает как OPTIONAL, то есть сервер общего
назначения вправе их не поддерживать. Если сервер метод не знает, он, как
правило, отвечает `501 (Not Implemented)`; если знает, но для этого ресурса
не разрешает — `405 (Method Not Allowed)`. Обе рекомендации спецификация
даёт модальностью SHOULD (§ 9.1), а не требованием. Требование стоит рядом:
в ответе `405` сервер **обязан** прислать поле `Allow` со списком
...
**Второе: действие, выбранное параметром запроса.** RFC 9110 § 9.2.1
разбирает этот случай отдельно и ставит на него требование уровня MUST. Если
ресурс существует ради небезопасного действия, владелец **обязан** запретить
это действие при обращении безопасным методом. Иначе обещание метода пустое.
Выноски ниже, но сначала сами: какая строка меняет состояние и что её
допускает?
...
Дальше работает не атакующий, а автоматика — та самая, что удалила черновик в
запросе, с которого тема началась. Листинг показывает тот же разрыв без
всякого трафика: намерение объявляет метод, действие выбирает параметр, и
никто их не сверяет. Что ответит этот обработчик на запрос
`DELETE /page` без параметров — и видно ли по ответу, что метод здесь вообще
ни при чём?
...
не сработает. Этот случай разбирается в теме `access-control-bypass-headers`.

**Цена.** REST (Representational State Transfer) API законно использует `PUT`, `DELETE` и `PATCH`, поэтому запрет
...
новом эндпоинте. Как правило, эту работу переносят в описание API и проверяют
автоматически.

## 9. Ловушка

Обычно думают: раз `GET` объявлен безопасным методом, обработчик `GET` можно
смотреть менее внимательно. Своё мнение — до опровержения: «безопасный» в
описании метода это обещание о намерении клиента или о поведении сервера?

**Это неверно.** «Безопасный» — обещание о намерении клиента, а не свойство
вашего кода. Цепочка ошибки такова: свойство запроса молча переносится на
обработчик, а из «клиент не просил менять» выводится «сервер не меняет».
...
Контрпример — тот же `page?do=delete`, с которого тема началась. Метод
безопасен по определению, обработчик удаляет страницу, а сканер, который
смотрит только на методы, ничего не сообщит.
...
перечислены методы, на которые сервер ответил кодом класса `2xx` или `3xx`.
Для каждого метода записано, ожидало ли его приложение.
...
лично 2026-08-22). Ни один пример не запускался: стенда под рукой не было. Сценарий с удалённым
черновиком — иллюстрация: механизм в нём тот, что описан в § 9.2.1, а
разобранного случая за ним нет.
