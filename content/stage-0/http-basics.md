...
То же в терминах, которыми это обсуждают. Запрос и ответ устроены одинаково:
...
остальное его дело. Потом по ссылкам пошли автоматы.
Поисковые обходчики и предзагрузка дёргают каждый найденный адрес, ничего не
спрашивая, и формальность перестала быть безобидной. RFC 9110
называет причину различения прямо: безопасные и небезопасные методы разделили
затем, чтобы обходчики и кеши работали без страха навредить. Обещание про намерение потому и обязывает
...
Сообщение приходит из-за границы доверия целиком. Метод, цель запроса, каждое
имя и значение поля, тело — всё выбирает атакующий. Между клиентом и
...
действие при обращении безопасным методом. До выносок: назовите, что здесь
сломано, и какой автомат это исполнит.
...
Из примера собирается правило темы: метод — заявление клиента о намерении, и
сверить заявление с действием может только сам обработчик. Проверьте на
соседнем случае: ту же ссылку «удалить черновик» оформили кнопкой, которая
отправляет `POST`, — что изменится для поискового обходчика?
...
**Цена.** REST (Representational State Transfer) API законно использует `PUT`, `DELETE` и `PATCH`. Запрет
...
Обычно думают: раз `GET` объявлен безопасным методом,
обработчик `GET` можно смотреть менее внимательно. Проверьте это убеждение до
чтения ответа. Обработчик отвечает только на `GET` и при каждом вызове
отправляет пользователю письмо с выгрузкой: можно ли такой обработчик
пропустить без вопросов, раз метод безопасный?
