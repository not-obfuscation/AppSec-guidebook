...
действие при обращении безопасным методом. Иначе обещание метода пустое.
Прежде чем читать выноски, найдите в листинге сами: какая строка меняет
состояние и что её допускает.
...
никто их не сверяет. Проверьте себя: что ответит этот обработчик на запрос
...
обработчик `GET` можно смотреть менее внимательно. Ответьте до опровержения:
«безопасный» в описании метода — это обещание о намерении клиента или о
поведении сервера?
