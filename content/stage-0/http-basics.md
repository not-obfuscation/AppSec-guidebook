...
**Зачем это в работе AppSec-инженера.** Обработчик HTTP-запроса вы писали не
раз. Метод в нём, скорее всего, ни разу не сверяли с тем, что код делает на
самом деле: фреймворк разбирает запрос, автор пишет логику. Ревью безопасности
начинается ровно на этом расхождении, а видно оно в одном артефакте — в
транскрипте запроса и ответа. Пока вы не читаете транскрипт сами, вы читаете
чужие выводы о нём: отчёт сканера, текст тикета, скриншот от разработчика.

**Что это такое.** Обмен по HTTP — это два текста: запрос от клиента и ответ от
сервера. В первой строке запроса стоит метод (`GET`, `POST`, `DELETE`) и адрес,
в первой строке ответа — трёхзначный код. Всё остальное — поля-заголовки и
тело.

Метод — не команда, которую сервер обязан исполнить, и не проверка, которую
кто-то делает за автора кода. Это ярлык, и пишет его клиент: объявление о том,
чего он якобы хочет. Совпадает ли ярлык с тем, что делает обработчик, не
проверяет никто — ни браузер, ни фреймворк, ни прокси. Из этого зазора растёт
всё дальнейшее.

**Пример: черновик удалился сам.** В админке есть ссылка «удалить черновик» —
обычная ссылка вида `<a href="/page?do=delete&id=1024">`. Утром черновика нет,
хотя на ссылку никто не нажимал. В логе за ночь — один запрос.
...
По ссылке прошла автоматика: предзагрузка в браузере, обходчик, проверка ссылок
в вики. Ни инъекции, ни обхода аутентификации здесь нет — сессия настоящая,
права настоящие, метод `GET` самый безобидный из восьми. Дефект в том, что
действие выбрано параметром `do`, а не методом, и чтение с записью стали
неразличимы для всех, кто смотрит на метод.

К этому запросу тема возвращается трижды. «Механика» разбирает, что обещает
метод и кому обещание адресовано. «Как выглядит в коде» показывает такой
обработчик изнутри, «Как чинится» — что в нём меняют.
...
- `access-control-bypass-headers` — обход контроля доступа сменой метода,
  параметра и поля.
