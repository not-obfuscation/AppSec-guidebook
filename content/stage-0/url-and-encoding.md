...
Случай задаёт правило всей темы: проверка стоит ровно столько, насколько парсер
проверки совпадает с парсером исполнения. Какой из двух хостов «настоящий» —
по строке не решить: ответ выбирает реализация.
...
Все пять компонентов видны на одной строке сразу:

```text
https://user:pass@example.com:8443/a/b?q=1#frag
```

Вот её разложение по компонентам:
...
запроса добавляет пробел, `"`, `#`, `<` и `>`. Стандарт отдельно оговаривает,
что набор запроса через набор фрагмента не определить: `` ` `` входит в набор
фрагмента, а в набор запроса не входит. Набор для пути расширяет набор запроса
символами `?`, `^`, `` ` ``, `{` и `}`; набор userinfo расширяет набор пути
ещё девятью символами.
...
Отсюда механизм двойного кодирования.
...
`bidiSkeleton(LTR, X)`. То есть `internalSkeleton` считается уже после того, как к строке
...
доверять стороне A — URL от A в какой-то момент могут прийти из
недоверенного источника.

## 5. Как выглядит в коде

**Первое и главное: два разных API разбора в одной цепочке.** Ревьюер ищет
пару «чем проверяли — чем пользовались». Найдите обе половины этой пары до
выносок.
...
Правка — одно слово во второй строке: `raw_url` заменён на `parsed`.
...
Имя, набранное целиком кириллицей, проверку «в строке одна письменность»
проходит. Исключает ли такая проверка подмену похожими символами?

**Нет.** Строка может быть целиком в одной письменности и всё равно
быть подделкой.
