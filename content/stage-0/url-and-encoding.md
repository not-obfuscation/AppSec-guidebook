...
**AG-PROTO-02** · уровень **L2** · время 30 мин (теория 15 / задача 10 /
самопроверка 5, оценка) · reviewed: 2026-08-22 · проверено на: RFC 3986
(STD 66, январь 2005), URL Standard (Living Standard, 18 августа 2026),
UTS #39 версии 17.0.0, ASVS v5.0.0
пререквизиты: `http-basics` · маппинг: CWE-172 · ASVS v5.0-1.2.2
...
обходов фильтров через кодирование разбирается в темах 1.2.03 и 1.8.01.

## 0. Коротко

RFC 3986 раскладывает URI на пять компонентов: схема, authority, путь,
запрос и фрагмент.
Одну и ту же строку по дороге от браузера до обработчика разбирают несколько
раз — на прокси, в фильтре, в маршрутизаторе фреймворка, в самом приложении, —
...
**Зачем это в работе AppSec-инженера.** Почти каждый обход фильтра сводится к
одной фразе: проверял один парсер, использовал другой. Пока вы не умеете
показать это на конкретной строке, спор с разработчиком упирается в «у меня
работает».
...
> **Примечание.** Источники расходятся в самой модели. WHATWG URL Standard
> § 4.1 определяет запись URL восемью полями — `scheme`, `username`,
> `password`, `host`, `port`, `path`, `query`, `fragment`, — и понятия
> authority среди них нет. Оба разложения верны: RFC 3986 описывает
> обобщённый синтаксис URI, WHATWG — структуру, которую строит браузер. Часть
> расхождений между парсерами берётся отсюда.

### Процентное кодирование

> **Примечание.** **Процентно-кодированный байт** — строка из символа `%` и
> двух шестнадцатеричных ASCII-цифр (URL Standard § 1.3).
...
запроса добавляет пробел, `"`, `#`, `<` и `>` — и специально оговорено, что
...
`internalSkeleton`, который для строки X выполняет четыре шага: приводит X к
нормальной форме NFD, удаляет символы со свойством
`Default_Ignorable_Code_Point`, заменяет каждый символ на его прототип из
таблицы confusables, снова применяет NFD. Сам `skeleton` определён через
направление письма: `skeleton(X)` — это `bidiSkeleton(LTR, X)`, то есть
`internalSkeleton` уже после того, как к строке применены переупорядочивание и
зеркалирование по двунаправленному алгоритму. Две строки считаются похожими
...
проходила проверку классом `URI` из Java, который видел хост `127.0.0.1`, а
на этапе загрузки, на части операционных систем и конфигураций, запрос уходил
на `127.0.0.1#.evilhost.com`. Разошлась трактовка фрагмента.
...
Признак дефекта здесь не в именах библиотек, а в том, что после проверки в
дело идёт `raw_url`, а не `parsed`.
...
Этого достаточно, чтобы расхождение между парсерами перестало иметь значение.
...
самодельные правила, а уровни ограничений: ASCII-Only, Single Script, Highly
Restrictive, Moderately Restrictive, Minimally Restrictive, Unrestricted.
...
отдельный алгоритм: перебрать строки, похожие на X, отбросить те, чьи
письменности пересекаются с письменностями X, и проверить, осталось ли среди
...
**Критерий выполнения** наблюдаемый: у вас есть таблица «строка × узел», в
ячейках — извлечённые хост и путь, и вы можете указать пальцем на строки, где
значения по узлам не совпали.
...
4. Потому что `/////evil.com/path` и `\\\evil.com/path` тоже начинаются со
   слеша или обратного слеша, а локальными путями не служат (CVE-2021-23401,
   при сервере WSGI, отличном от Werkzeug, либо при
...
- Тема 1.2.03 — обход фильтров SSRF альтернативными представлениями адреса.
- Тема 1.8.01 — path traversal и обходы фильтров пути.
...
взят из отдельного источника. Ни один пример не запускался: стенда у миссии
нет.
