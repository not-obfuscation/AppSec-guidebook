...
Пять компонентов URI из RFC 3986
...
а не по исходной строке. То же в терминах, которыми это обсуждают.
...
пару «чем проверяли — чем пользовались». Найдите оба её конца в листинге, не
заглядывая в выноски.
...
`raw_url`: так и прошёл адрес из начала темы. Изменится ли признак, если
вместо двух библиотек окажутся две версии одной?
...
Правило выглядит достаточным: если запретить смешение письменностей в имени,
подмена похожими символами закрыта. Проверьте себя: верно ли это
представление?
