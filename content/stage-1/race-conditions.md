...
**Зачем это в работе AppSec-инженера.** MITRE держит для класса номер
CWE-362, а для проверки отдельно от применения — CWE-367; издание OWASP
2025 года относит CWE-362 к категории `A06:2025` Insecure Design. Тема
даёт короткий вопрос к любому обработчику с пределом: чем предел удержан,
если два таких запроса придут в одну миллисекунду. Ответ читается по числу
операций между чтением и записью.
...
- `idempotency` — повторённый запрос, у которого столкновение не случайно, а
  задумано клиентом.
- `money-rounding` — что происходит с суммой, когда две правки всё-таки
  сошлись и результат надо сверить.
- `business-logic-flaws` — правило предметной области, которое здесь снимают
  одновременностью.
- `multistep-flow-checks` — порядок шагов и состояние процесса со стороны
  контроля доступа.
