...
SSRF — дефект, при котором адрес исходящего запроса приложения задаёт тот, кто
прислал запрос входящий. Атакующий подменяет адрес и получает сервер в роли
посредника. Тот ходит туда, куда самому атакующему хода нет: на петлю, во
внутреннюю сеть, на служебные интерфейсы платформы. Опасно это не самим
запросом, а тем, что внутренние службы обычно не спрашивают, кто их зовёт:
их защищала недоступность снаружи, и она перестала работать. Чинится тем, что
адрес собирает приложение по списку разрешённого, а не клиент.

**Зачем это в работе AppSec-инженера.** Каталог MITRE держит для этого дефекта
отдельный номер, CWE-918, а OWASP относит его к A01:2025 вместе с прочими
нарушениями контроля доступа. В ревью SSRF ищется одним вопросом к каждому
исходящему запросу: кто задал адрес. Вопрос задаётся загрузчику аватара по
ссылке, обработчику вебхука, генератору отчётов, импортеру и любой интеграции,
которая ходит по адресу из настроек пользователя.
...
Свод требует от каждого разбора одного набора подцелей, и SSRF раскладывается
по нему без остатка.
...
передаёт его клиенту HTTP. **Sink** — вызов клиента: `requests.get`, `fetch`,
...
называет CSRF — запрос отправляет браузер жертвы. Ценность приёма в том, что к
...
**Внутренняя сеть.** Службы, до которых пользователю хода нет, стоят на
адресах вида `10.0.7.5` или `192.168.0.68`. Их защищала топология сети, поэтому
проверок в них обычно меньше: внутренний API отчётов отдаёт всё, что попросят,
без единого вопроса о правах.
...
функция `url()` в стилях. Запрос делает не код приложения, а библиотека
разметки, и в обзоре кода этот сток видно хуже всего.
...
```text
$ python3 hack.py
  опыт 1 — петля, 127.0.0.1:8080: код 200 — СЕССИЯ АДМИНИСТРАТОРА
  опыт 2 — внутренняя сеть, 10.0.7.5: код 200 — ЧУЖОЙ СЧЁТ ПОЛУЧЕН
  опыт 3 — метаданные, 169.254.169.254: код 200 — УЧЁТНЫЕ ДАННЫЕ РОЛИ
  опыт 4 — схема file:: код 200 — ФАЙЛ НАСТРОЕК ПОЛУЧЕН
  опыт 5 — партнёр перенаправляет внутрь: код 200 — МЕТАДАННЫЕ
...
Опыт 2 — второй круг. Служба счетов не спрашивает ни о правах, ни об имени
вызывающего: её защищала недоступность из интернета.
...
имя клиента, механизм не меняется.
...
список мест, куда ему положено ходить, и строит адрес само.
...
$ LAB_TARGET=solution.py python3 hack.py
  ...
  опыт 5 — партнёр перенаправляет внутрь: код 400
  опыт 6 — контроль, карточка партнёра: код 200 — карточка отдана
```
...
в обход `allowed_url`. Он пишется не тестом, а правилом из блока 8, и потому
работает на каждом коммите.
...
Распространённое представление: приложение по замыслу ходит по ссылке, которую
дал пользователь, — предпросмотр, импорт, вебхук. Значит, исходящий запрос
здесь функция, а не дефект, и говорить не о чем.
