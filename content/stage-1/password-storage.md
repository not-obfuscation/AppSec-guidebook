...
asvs: ['v5.0-11.4.2', 'v5.0-6.2.5', 'v5.0-6.2.8', 'v5.0-6.2.9', 'v5.0-6.2.12', 'v5.0-6.5.2']
...
**AG-AUTH-07** · уровень **L1** · время 90 мин (теория 40 / лаба 35 /
самопроверка 15, оценка) · reviewed: 2026-08-22 · проверено на: Python 3.14.7
(OpenSSL), Node.js v26.7.0, Semgrep 1.174.0
пререквизиты: `app-architecture`, `sessions-vs-tokens` · маппинг: CWE-916 ·
CWE-759 · CWE-760 · CWE-328 · ASVS v5.0-11.4.2 · WSTG-v42-CRYP-04 · A07:2025

Состав блоков — полный по уровню L1 (`PLAYBOOK.md` 3.1): удалений нет, порядок
канонический. Учебный план держит тему в подразделе «Authentication Failures»,
а сам OWASP Top 10 относит слабое хранение паролей к A04:2025 и пишет об этом
прямо в описании A07:2025. Обе строки маппинга стоят в шапке.
...
и после утечки дампа перебор идёт со скоростью миллионов проверок в секунду на
...
Источник — пароль пользователя. Sink — строка в таблице учётных записей.
...
файла. Для пароля скорость становится недостатком, и Password Storage Cheat
Sheet формулирует это прямо: быстрые функции не подходят, потому что позволяют
атакующему делать много догадок за короткое время.

Порядок величин видно замером. На AMD Ryzen 7 9800X3D, Python 3.14.7, один
поток:
...
локально на стандартной библиотеке Python, сети не требует. Приёмы ниже
привязаны к этому стенду и применимы только к нему.
...
Пересечение двух списков и даёт находку этой темы.
...
пароль нужен приложению для входа в чужую систему.
...
**Argon2id в обеих стандартных библиотеках отсутствует.** Cheat Sheet ставит
Argon2id первым выбором, но ни `hashlib`, ни `node:crypto` его не содержат.
Взять его — значит завести внешнюю зависимость в контур аутентификации, и это
решение уровня архитектуры, а не строки кода. Поэтому в примерах выше стоит
scrypt: он второй по списку Cheat Sheet и доступен без зависимостей.
...
- Тема 1.6.08 — password spraying и credential stuffing: атака на живой вход,
  а не на утёкший дамп.
- Тема 1.6.01 — брутфорс и защита от него: цена догадки со стороны сети.
- Тема 1.11.02 — слабые криптоалгоритмы за пределами паролей.
...
**Число источников.** Шесть собственных против потолка в три. Четвёртый
...
правило двух стеков из части 4 свода: парные листинги требуют документации
стеков, и до этой темы в реестре не было ни одной такой записи для паролей.
...
2026-08-22. Поведение Argon2id **не проверялось**: реализации в стандартных
библиотеках нет, внешнюю зависимость в пилот не заводили.
