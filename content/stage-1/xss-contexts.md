...
значение и что с ним по дороге сделали.

### Разбор по общей рамке

**Источник** — присланное значение. **Путь** — код, доводящий его до шаблона.
**Sink** — конкретное место разметки: тело, атрибут, адрес, скрипт, стиль.
...
шестнадцатью знаками — как есть. Нагрузку это остановит, а законное значение с
...
Ревьюер смотрит не на вызов, а на разметку вокруг подстановки.
...
2. Verify that значения всех атрибутов заключены в кавычки, включая те, куда
   подставляется число.
3. Verify that адрес, собранный из присланного значения, проверен по схеме и
