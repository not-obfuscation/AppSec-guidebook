...
**AG-AC-02** · уровень **L1** · время 90 мин (теория 40 / лаба 35 /
самопроверка 15, оценка) · reviewed: 2026-08-23 · проверено на: Python 3.14.7,
Semgrep 1.174.0, WSTG v4.2, OWASP Top 10 издания 2025, Spring Security
(живая документация), ASVS v5.0.0, CWE 4.20
пререквизиты: `access-control-models`, `sessions-vs-tokens` · маппинг:
CWE-862 · CWE-425 · CWE-276 · ASVS v5.0-8.2.1 · WSTG-v42-ATHZ-03 · A01:2025

Состав блоков — полный по уровню L1 (`PLAYBOOK.md` 3.1): удалений нет, порядок
канонический. Подмена признака роли в запросе разобрана отдельной темой
`role-parameter-tampering`: здесь атакующий ничего не подделывает, он приходит
туда, где его не ждали.

## 0. Коротко

Вертикальная эскалация — доступ к функции, положенной более высокой роли.
Обычный пользователь открывает административную страницу, выгружает базу,
удаляет чужую запись. Причина почти всегда одна: охрана знает про часть
функций, а не про все, потому что список закрытого ведут руками, а маршруты
добавляют кодом. Чинится отказом по умолчанию — маршрут без объявленного права
недоступен — и тем, что охрана и маршрутизатор разбирают запрос одинаково.

**Зачем это в работе AppSec-инженера.** Это первый пункт первой категории
OWASP Top 10 и первый вопрос на собеседовании про контроль доступа. В работе
он даёт находки на каждом втором ретесте: список закрытого отстаёт от кода
всегда, вопрос лишь в том, на сколько маршрутов. Ревьюер, который умеет
показать разрыв одной командой, экономит команде спор о серьёзности.
...
Свод требует от каждого разбора одного набора подцелей, и вертикальная
...
**Что правило пропустит.** Всё, что не названо `admin`: выгрузка под
`/reports/export` из опыта 1 остаётся невидимой, хотя она и есть главная
находка темы. Всякую охрану, устроенную списком путей, а не объявлением у
...
Распространённое представление: если административная страница не показана в
интерфейсе и её адрес нигде не напечатан, доступ к ней закрыт — угадать адрес
нельзя.
