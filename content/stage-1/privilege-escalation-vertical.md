...
Роли делят функции приложения: то, что позволено администратору, обычному
пользователю не положено. Граница между ними держится не сама — её держит
код, который перед вызовом обработчика спрашивает о праве. Вертикальная
эскалация — это когда обычный пользователь попадает в функцию
администратора, потому что спросить о праве оказалось некому.

Пример. В портале заявок охрана закрывает `/admin/users` и
`/admin/tickets/delete`; выгрузка всех заявок объявлена по адресу
`/admin/export`, и в список охраны он не входит. Вот запрос к ней от
сессии обычного пользователя:
...
`user`, и запрос её не меняет. Адрес тоже настоящий: его объявил разработчик.
...
Вертикальная эскалация — доступ к
...
разбирают запрос одинаково. То же в терминах, которыми это обсуждают.

**Зачем это в работе AppSec-инженера.** Маршрут вы добавляли: строка в роутере,
обработчик, тесты зелёные. Список закрытых путей лежит в другом файле, и
дописать в него строку никто не напоминает. Это первый пункт первой категории
...
**Сток** — сам обработчик: функция, выполняющая чувствительное действие.
...
отказом по умолчанию: охрана спрашивает про объявленное право, а не про список,
...
Ревьюер ищет не проверку, а **умолчание**: что случится с маршрутом, о
котором никто ничего не объявил. Вопрос задаётся коду, а не документации.
Проследите сами, каким путём запрос к `/admin/export` проходит мимо охраны,
и назовите строку, где он мог бы остановиться.
...
Эта пара устроена как вся тема: находку делает не отсутствие проверки,
а умолчание охраны — маршрут (3) проходит потому, что про него ничего не
объявлено. Прежде чем листать дальше, прикиньте: какая одна правка в этом
фрагменте сделала бы дефект невозможным при любом содержимом `PROTECTED`?
...
первое совпадение**. До чтения выноски ответьте: какой маршрут из механики
откроется здесь любому вошедшему?
...
маршрут без объявления закрыт. Листинг читается в три приёма. Первый — найти
место, где маршрут без права отвергается ещё при старте. Второй — увидеть,
где путь приводится к канону, общему для маршрутизатора и охраны. Третий —
проверить, где сама охрана отвечает отказом на отсутствие правила.
...
административной быть не перестала. Всякую охрану, устроенную списком путей, а
не объявлением у маршрута. Любой стек, где маршруты заводятся не декоратором.
...
Вопрос до разбора: закроет ли находку переименование выгрузки в адрес, который
нельзя угадать? Выгрузка из введения лежала по угадываемому адресу, и первый
ответ, который приходит на такую находку, — переименовать. Отсюда
представление: если административная страница не показана в интерфейсе и её
адрес нигде не напечатан, доступ к ней закрыт — угадать адрес нельзя.
...
4. Verify that проверка права происходит в доверенном слое службы, недоступном
   вызывающей стороне (ASVS v5.0-8.3.1).
