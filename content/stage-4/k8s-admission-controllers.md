...
Запрос к API-серверу проходит аутентификацию, авторизацию (RBAC, ролевая
модель доступа — тема `k8s-rbac`) и цепочку admission-контроллеров;
записывается объект только
...
Синхронный вызов — на пути записи.
...
изменение. Что сломается, если валидирующие пойдут первыми?
