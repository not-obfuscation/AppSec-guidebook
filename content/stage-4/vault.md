...
переменной окружения, но не смена модели. Отсюда и выбор движка для
лабы.
...
Третий ответ — создание статической роли — заодно сменил пароль
...
Пароль при этом не длиннее жизни аренды ни на минуту. Без этих двух
полей ответ сводился бы к паролю.
...
Сопоставьте со своим сервисом: какой срок годности у учётки, которой
оно ходит в базу, — и кто этот срок назначил?
...
Секреты переехали в хранилище — ушла ли с ними и утечка? Слышно и
такое: раз секреты в Vault, утечки больше нет.
...
В проверялке стоит динамическая роль: один запрос «дай пароль
к базе» дважды даёт две разные учётки, и отзыв гасит учётку в базе.
Статической роли в лабе нет — её прогон остался в тексте темы выше.
Рабочий цикл такой: поднять стенд, сбросить его, прочитать бланк
