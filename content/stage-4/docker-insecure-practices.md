...
teaches:
  ...
  - Найти в описании запуска четыре небезопасные практики
...
флаг --privileged, сокет демона внутри контейнера и лишние
capabilities. У каждой своя цена и своя починка, но читаются они одинаково:
...
То же в терминах, которыми это обсуждают. Контейнер — процесс хоста с
подрезанными обзором и полномочиями; что именно подрезано, решает
описание запуска. Флаг --privileged снимает почти всё: полный набор
capabilities, устройства хоста, без фильтра вызовов. Смонтированный
/var/run/docker.sock даёт контейнеру управление демоном, а демон работает
от root. Root в контейнере без user namespace — это root на хосте по
карте uid. Чинится срезом: свой пользователь, cap-drop=all с точечным
добавлением, никакого сокета внутри, лимиты и read-only.
...
4. Найти в описании запуска четыре небезопасные практики.
...
пока изоляция не снята флагом запуска. Три практики снимают её в трёх
разных местах: --privileged — сразу всё, --cap-add — точечно, сокет
демона — обходом через доверенный процесс.
...
попросить демона запустить привилегированный контейнер.
...
docker run в CI. Четыре признака:
...
Те же четыре признака в команде запуска:
...
или ошибка, решает контекст. Замечание же по форме: DL3006 про тег
...
стоять по разные стороны границы.
...
4. Почему cap_add с одним именем — не находка, а --privileged —
   находка?
5. Возврат к теме `linux-capabilities`: что за бит добавился в CapEff
   при --cap-add SYS_ADMIN и как это видно по маске?
...
5. Бит CAP_SYS_ADMIN: маска выросла с a80425fb до a82425fb, разница
   ровно в одном бите — 0x200000.
