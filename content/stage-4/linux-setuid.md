...
Бит setuid на исполняемом файле подменяет при запуске действующий uid
процесса на uid владельца файла; setgid делает то же с группой. У
процесса при этом три идентификатора: реальный — кто вызвал,
действующий — с чьими правами идут проверки, сохранённый — куда можно
вернуться. На скрипте бит игнорируется, на каталоге setgid означает
наследование группы, а sticky bit на общем каталоге запрещает удалять
чужие файлы. Опасность сосредоточена в setuid-программе: она работает с
чужими правами и при этом наследует окружение вызывающего, поэтому
обязана собирать его заново по белому списку. То же в терминах,
...
каждый может стереть чужое; с ним `/tmp` безопасно общий. Шаг 3 блока
«Эксплуатация» показывает это на `/tmp`.
...
всего признак ошибки при назначении. Встретили прописную — проверьте
назначение.
...
Прогон сводится к одному правилу: всё, что процесс унаследовал,
выбирает вызывающий. Ответа в транскрипте нет: обёртку переписали на C
и собрали статически — какие входы у атакующего останутся?
...
**Первое: поиск программы по чужому PATH.** Из лабы — найдите строку,
через которую вызывающий выбирает исполняемый файл, прежде чем читать
выноску:
...
**Второе: то же на C.** Здесь два дефекта, а не один: попробуйте
назвать оба. Классическая обёртка из админских скриптов:
...
Сброс привилегий одним seteuid(getuid()) обратим: сохранённый uid
остаётся нулём, и права возвращаются вызовом seteuid(0). Полный сброс
делает setuid(getuid()): у привилегированного процесса он переписывает
все три идентификатора разом — порядок разбирается в credentials(7),
и ошибка там стоит отдельного дефекта.
...
Оцените сами: обёртка с setuid работает от служебной учётной записи,
не от root. Такой запуск уже безопасен?

Оценка «безопасен» опирается на сравнение с root: setuid от
служебного пользователя безопасен — это же не root.
...
файлам и процессам этого владельца. Заблуждение строится на молчаливой
подмене цели: будто атакующему нужен только root. На практике ему нужен
чужой ресурс, и права служебной учётной записи его уже открывают.
...
4. Нет. LD_PRELOAD до хелпера не доедет: реальный и действующий uid
   у него разные, и загрузчик такие переменные игнорирует (`AT_SECURE`).
   Доезжает остальное окружение, и выбирает его вызывающий. Абсолютный
   путь закрывает выбор файла, но не окружение.
