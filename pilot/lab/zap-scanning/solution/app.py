...
Это РЕШЕНИЕ задачи «почини»: шесть проб hack.py закрыты, tests.py зелёный.
Дефекты, которых пробы не касаются (доступ к чужому заказу, роль, знак
количества, заголовки, флаги cookie), здесь намеренно оставлены — их закрытие
в задачу не входит.
...
def safe_data_path(name):
    """Имя файла из каталога data и ничего кроме. Пусто — если не подходит."""
    if not re.fullmatch(r"[A-Za-z0-9._-]{1,64}", name or "") or name.startswith("."):
        return ""
    full = os.path.realpath(os.path.join(DATA, name))
    return full if full.startswith(os.path.realpath(DATA) + os.sep) else ""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def fail(self, exc):
        """Наружу — общее слово, подробности в журнал сервера."""
        log_line("ОШИБКА\t%s\t%s: %s" % (self.path, type(exc).__name__, exc))
        self.send_html("ошибка", "<p>Не получилось. Попробуйте ещё раз.</p>", 500)
    ...
    def route_get(self, path, qs):
        ...
        if path == "/search":
            ...
            return self.send_html(
                ...
                "<button>найти</button></form>" % html.escape(q),
            )
        ...
        if path == "/catalog":
            ...
            con = db()
            rows = con.execute(
                "SELECT name, price FROM products WHERE cat = ?", (cat,)
            ).fetchall()
            body = "<ul>" + "".join(
        ...
        if path == "/files":
            ...
            full = safe_data_path(name)
            if not full or not os.path.isfile(full):
                return self.send_html("нет файла", "<p>Файл не найден.</p>", 404)
        ...
        if path == "/profile":
            ...
            return self.send_html(
                ...
                "<input name='note'><button>сохранить</button></form>"
                % html.escape(note),
            )
        ...
        if path == "/api/v1/products":
            ...
            con = db()
            rows = con.execute(
                "SELECT id, name, price FROM products WHERE cat = ?", (flt,)
            ).fetchall()
