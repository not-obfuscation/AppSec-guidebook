"""Модель агента netprobe: полный набор capabilities вместо одной.

netprobe — диагностический агент: раз в минуту он проверяет узлы
сырым сокетом ICMP. В описании его запуска оставили «root, на всякий
случай», то есть полный набор capabilities, хотя из сорока одной
агенту нужна ровно одна — cap_net_raw.

Это модель: процесс — структура данных с набором capabilities,
системные вызовы — функции, проверяющие набор там, где его проверяло
бы ядро. Никакие настоящие привилегии не используются и не требуются.

Лаборатория гайдбука. Всё исполняется локально, сети не требует и
никуда не обращается. Применимо только к этой лабе.

Задача: починить запуск агента так, чтобы hack.py перестал
...
def start_agent(targets: dict) -> Agent:
    """Запуск агента."""
    # УЯЗВИМО — демонстрация, не для продакшена: агенту выдали весь
    # набор root, хотя для работы ему нужна ровно одна capability.
    proc = Process("netprobe", effective=ALL_CAPS)
